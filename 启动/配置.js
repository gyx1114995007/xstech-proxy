...
const 配置 = {
  ...
  xstech: {
    ...
    发送文件开关字段: String(process.env.XSTECH_SEND_FILE_FLAGS || '').toLowerCase() === 'true',
  },
  会话池: {
  ...
  responses文件上下文: {
    mode: process.env.RESPONSES_FILE_CONTEXT_MODE || 'auto',
    ttlMs: parseInt(process.env.RESPONSES_FILE_CONTEXT_TTL_MS || '3600000', 10),
  },
  openai文件提取: {
    // last_user：默认只提取最后一条用户输入里的图片/文件，避免全量上下文客户端反复触发上游识别。
    // all：严格提取完整历史中的所有图片/文件；仍受转换层限制：最多 8 个文件，单文件最大 10MB。
    chatScope: process.env.OPENAI_CHAT_FILE_SCOPE || 'last_user',
    responsesScope: process.env.RESPONSES_INPUT_FILE_SCOPE || 'last_user',
  },
