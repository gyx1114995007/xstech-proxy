...
const 池 = {};
const 账号状态 = {};
const 会话对象缓存 = new Map(); // sessionId -> { data: 完整会话对象, cachedAt: 时间戳, lastAccessedAt: 最后访问时间 }
const 缓存上限 = Math.ceil(池大小上限 * 1.2); // 池上限的1.2倍，留点余量
let 上次全量同步时间 = 0;
let 缓存统计 = { 命中次数: 0, 未命中次数: 0, 淘汰次数: 0 };
...
/**
 * LRU淘汰：当缓存超过上限时，删除最久未访问的
 */
function LRU淘汰() {
  if (会话对象缓存.size <= 缓存上限) return;
  
  // 找出最久未访问的
  let 最老的Key = null;
  let 最老的时间 = Date.now();
  
  for (const [key, value] of 会话对象缓存.entries()) {
    const lastAccess = value.lastAccessedAt || value.cachedAt;
    if (lastAccess < 最老的时间) {
      最老的时间 = lastAccess;
      最老的Key = key;
    }
  ...
  if (最老的Key) {
    会话对象缓存.delete(最老的Key);
    缓存统计.淘汰次数++;
    日志.debug('会话池', `LRU淘汰: ${最老的Key}, 缓存大小: ${会话对象缓存.size}/${缓存上限}`);
  }
}
...
function 获取缓存统计() {
  const 全量同步间隔 = 获取全量同步间隔();
  const 总请求 = 缓存统计.命中次数 + 缓存统计.未命中次数;
  const 命中率 = 总请求 > 0 ? Math.round((缓存统计.命中次数 / 总请求) * 10000) / 100 : 0;
  
  return {
    缓存数量: 会话对象缓存.size,
    缓存上限: 缓存上限,
    使用率: 会话对象缓存.size > 0 ? Math.round((会话对象缓存.size / 缓存上限) * 100) : 0,
    命中次数: 缓存统计.命中次数,
    未命中次数: 缓存统计.未命中次数,
    命中率: 命中率 + '%',
    淘汰次数: 缓存统计.淘汰次数,
